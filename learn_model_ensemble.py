...
def main(
    ...
    seed: int = 0,
    exp_name: str = "ensemble_default_1e-5_whole_dataset/",
    ...
    assert partition < n_ensembles
    # make the results folder
    os.makedirs(f"data/model_learning/{exp_name}/{partition}", exist_ok=True)
    os.makedirs(f"data/model_learning/{exp_name}/{partition}/figures", exist_ok=True)
    ...
    random.seed(seed)
    partition_seeds = [random.randint(0, 2**32-1) for _ in range(n_ensembles)]
    random.seed(partition_seeds[partition])
    ...
    datasubset = torch.utils.data.Subset(dataset, subset)
    data_loader = torch.utils.data.DataLoader(datasubset, batch_size=batch_size, shuffle=False, drop_last=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 5 == 0:
            ...
            # save all models
            os.makedirs(f"data/model_learning/{exp_name}/{partition}/epoch_{epoch}", exist_ok=True)
            torch.save(representation_net, f"data/model_learning/{exp_name}/{partition}/epoch_{epoch}/representation_net.pt")
            torch.save(dynamics_net, f"data/model_learning/{exp_name}/{partition}/epoch_{epoch}/dynamics_net.pt")
            torch.save(reconstruction_net, f"data/model_learning/{exp_name}/{partition}/epoch_{epoch}/reconstruction_net.pt")
    
    # save model
    torch.save(representation_net, f"data/model_learning/{exp_name}/{partition}/representation_net.pt")
    torch.save(dynamics_net, f"data/model_learning/{exp_name}/{partition}/dynamics_net.pt")
    torch.save(reconstruction_net, f"data/model_learning/{exp_name}/{partition}/reconstruction_net.pt")

    # save losses together
    np.savez_compressed(f"data/model_learning/{exp_name}/{partition}/losses.npz",
...
if __name__ == '__main__':
    ...
    # Use -1 for all partitions, else partition is the index of the subnetwork to train
    parser.add_argument("--partition", "-p", type=int, default=-1)
    parser.add_argument("--epochs", "-e", type=int, default=15)
    parser.add_argument("--num_ensembles", "-n", type=int, default=5)
    parser.add_argument("--seed", "-s", type=int, default=42)
    args = parser.parse_args()
    if args.partition == -1:
        for i in range(args.num_ensembles):
            main(exp_name=args.output, data_path=args.data, partition=i, num_epochs=args.epochs, n_ensembles=args.num_ensembles, seed=args.seed)
            # Clear memory
            torch.cuda.empty_cache()
            gc.collect()
    else:
        main(exp_name=args.output, data_path=args.data, partition=args.partition, num_epochs=args.epochs, n_ensembles=args.num_ensembles, seed=args.seed)
