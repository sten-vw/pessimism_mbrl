...
# replay buffer
state_trajectories = torch.zeros((NUM_FRAMES, 10, 10, 4), dtype=torch.bool, device=device)
action_trajectories = torch.zeros((NUM_FRAMES, 1), dtype=torch.int32, device=device)
reward_trajectories = torch.zeros((NUM_FRAMES, 1), dtype=torch.float32, device=device)
terminal_trajectories = torch.zeros((NUM_FRAMES, 1), dtype=torch.bool,)
...
#   output_folder: directory and file name prefix to output data and network weights, file saved as 
#       <output_folder>_data_and_weights
#   store_intermediate_result: a boolean, if set to true will store checkpoint data every 1000 episodes
#       to a file named <output_folder>_checkpoint
#   load_path: file path for a checkpoint to load, and continue training from
#   step_size: step-size for RMSProp optimizer
#
#################################################################################################################
def dqn(env, replay_off, target_off, output_folder, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE, seed=0):
    ...
    print("seeds (general, np, env, torch):", seed, np_seed, env_seed, torch_seed)
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if replay_off:
                ...
                reward_trajectories[t] = reward
                terminal_trajectories[t] = is_terminated
        ...
        returns += G
        if e % 10000 == 0:
            logging.info("Episode " + str(e) + " | Return: " + str(G) + " | True avg return: " + str((returns/1000)) + " | Avg return: " +
                         str(numpy.around(avg_return, 2)) + " | Frame: " + str(t) + " | Time per frame: " + str((time.time()-t_start)/t)  + " | Elapsed time: " + str((time.time()-t_start)) )
            returns = 0

            np.savez_compressed(output_folder + "/experience_data_episode_" + str(e) + ".npz",
        ...
        if store_intermediate_result and e % 1000 == 0:
            ...
            }, output_folder + f"/episode_{e}_checkpoint")
    ...
    torch.save({
        ...
        'policy_net_state_dict': policy_net.state_dict()
    }, output_folder + "/data_and_weights")
    torch.save(policy_net, output_folder + "/policy_net.pt")

    np.savez_compressed(output_folder + "/experience_data_full"+".npz",
        states=state_trajectories.cpu().numpy(),
        actions=action_trajectories.cpu().numpy(),
        rewards=reward_trajectories.cpu().numpy(),
        terminals=terminal_trajectories.cpu().numpy())
