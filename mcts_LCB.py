...
class Node:
    def __init__(self, action, state, reward, continue_probability, num_actions=3, parent=None, env=None):
        # after state
        self.state = state
        self.action = action
        self.parent = parent
...
def MCTS(root: Node, 
    ...
    for _ in range(num_simulations):
        ...
        if depth < planning_horizon:
            ...
            if node.env is None:
                state_reconstructed = model.decode(node.state)
                new_env = None
            else:
                state_reconstructed = node.env.state()
            ...
            for _ in range(depth, planning_horizon):
                ...
                if temp_env is None:
                    state_reconstructed = model.decode(old_state)
                else:
                    state_reconstructed = temp_env.state()
                    temp_env.act(action)
                state_action = (state_reconstructed.tobytes(), action)
                penalty = L / np.sqrt(state_action_map.get(state_action, 1))
                rollout_discounted_return += (reward - penalty) * cumulative_discount_factor * cumulative_continuation_probability
